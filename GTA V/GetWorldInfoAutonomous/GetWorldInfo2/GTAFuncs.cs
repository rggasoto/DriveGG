...
namespace DeveloperConsole {
    public static class GTAFuncs {
        ...
        public static Vector3 RotationToDirection(Vector3 rotation)
        {
            ...
            float num = Math.Abs((float)Math.Cos((double)x));
            return new Vector3
            {
                X = (float)((double)((float)(-(float)Math.Sin((double)z))) * (double)num),
                Y = (float)((double)((float)Math.Cos((double)z)) * (double)num),
                Z = (float)Math.Sin((double)x)
            };
        }
