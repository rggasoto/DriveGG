...
def loadcsv(name,discard_zero):
    ...
    for i in range(len(lines)):        
        ...
        # Input range: 0 - 254
	# Center 127
        steer = ((float(row[steering])-127)/127.0*35.0)/180.0*pi
...
files = ['training.csv','training.csv']#,'training.csv','training.csv','training.csv']
#Hyper parameters for each training
discard = [True,False,True,True,True]
...
with open('model_new_5.json', 'w') as f:
    f.write(model.to_json())
model.save_weights('model_new_5.h5')
